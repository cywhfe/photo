...
$(document).ready(function(){
	...
	$("#radio").formValidator({onShow:"请输入radio,限制数字",onFocus:"请输入radio,限制数字",onCorrect:"输入正确"}).inputValidator({min:0,max:10,onError:"radio最多输入10个字符,请确认"}).regexValidator({regExp:"[0-9]{0,10}$",onError:"只能输入数字"});
	...
	//flag,info,back_url,parent_id,radio,show_order,status,start
  });
...
.tbldt-bd td {
	...
	<div class="page-content clearfix">
		...
		<form action="${ctx}/parameter/visitFromEdit.action" method="post" id="dataForm" onkeydown="if(event.keyCode==13){return false;}">
		<input id="id"  type="hidden"   name="visitFrom.id" value="${visitFrom.id}"/>
		<div class="c-right fl">
			...
			<tbody class="tbldt-bd">
					...
					<tr>
						...
						<td style="width:200px" align="left">
						<input  id="radio" type="text"   onkeyup= "value=value.replace(/[^\d]/g, '')" name="visitFrom.radio" class="ex-searchinput" style="width: 200px"   value="${visitFrom.radio}"/>
						</td>
					...
					<tr>
						...
						<td style="width:300px" align="left">
							有效<input id="status2"  type="radio" class="ex-searchinput" style="width: 20px;margin-top:-3px" name="visitFrom.status" value="1" <c:if test='${1==visitFrom.status}'>checked="checked"</c:if> /> 
							无效<input id="status1"  type="radio" class="ex-searchinput" style="width: 20px;margin-top:-3px" name="visitFrom.status" value="0" <c:if test='${0==visitFrom.status}'>checked="checked" </c:if> />
						</td>
					...
					<tr>
						...
						<td style="width:200px" align="left">
						<input   id="start"  onkeyup= "value=value.replace(/[^\d]/g, '') " type="text"  name="visitFrom.start" class="ex-searchinput" style="width: 200px"   value="${visitFrom.start}"/>
						</td>
